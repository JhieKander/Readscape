...
<?php 
     ...
     $singlecategory = $book->getCategorySingle($category_id);
     $category = $book->getCategory();
     $book->updateCategory();
    $book->archiveCategory();

    ?>
  <?php foreach ($singlecategory as $displaycat){$category_name =  $displaycat['category_name'];}?>
            ...
            <h5>Categories</h5>
                <div class="row">
                  ...
                  <form method="POST" action="">
                   <input type="hidden" name="category_id" value="<?=$displaycat['category_id'];?>">  
                    <div class="row">
                       <div class="col-md-5">
                          <input type="hidden" name="encoder" value="<?=$admininfo['firstname']?> <?=$admininfo['lastname']?> ">
                          <input class="form-control" type="text" name="category_name" placeholder="Enter a Category" required value="<?=$category_name?>">
                        </div>
                        <div class="col-md-5">
                            <button type="submit" name="update_cat" class=" text-light btn btncategory">Update Category</button>
                        </div>
                    </div>
                  </form>
                 </div>
